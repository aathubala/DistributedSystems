...
router.put("/update/order/status/:id", async (req, res) => {
  ...
  try {
      ...
      await Item.findByIdAndUpdate(enrollId, updateisDelivered)
...
router.route("/accepted").get(async (req, res) => {
  try {
    // Find items where accepted is true
    const acceptedItems = await Item.find({ accepted: true });

    // Send the accepted items as a response
    res.status(200).json(acceptedItems);
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: "Error fetching accepted items", error });
  }
});
